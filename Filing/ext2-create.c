...
void write_superblock(int fd) {
    ...
    superblock.s_max_mnt_count = UNLIMITED_MNT_COUNT;
    superblock.s_magic = EXT2_SUPER_MAGIC;
    superblock.s_state = EXT2_VALID_FS;
    superblock.s_errors = EXT2_ERRORS_CONTINUE;
    superblock.s_minor_rev_level = 0;
    superblock.s_lastcheck = current_time;
    superblock.s_checkinterval = CHECK_INTERVAL;
    superblock.s_creator_os = EXT2_OS_LINUX;
    superblock.s_rev_level = 0;
    superblock.s_def_resuid = EXT2_DEF_RESUID;
    superblock.s_def_resgid = EXT2_DEF_RESGID;
...
void write_block_bitmap(int fd)
{
    ...
    if (write(fd, bit_map, BLOCK_SIZE) != BLOCK_SIZE)
    {
        errno_exit("write");
    }
...
void write_hello_world_file_block(int fd)
{
    ...
    ssize_t size = sizeof(txt);
    if (write(fd, txt, size) != size) {
        errno_exit("write");
    }
}
